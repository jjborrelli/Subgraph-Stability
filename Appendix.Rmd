...
nd <- function(gl) nrow(gl) - nrow(unique(aaply(gl, 1, sort))) # determines the number of double links
...
```{r eval = F}
# Use the filepath.sink variable to set the path for storing the sink information 
filepath.sink <- my.sink.path
# Use the filepath.data variable to set the location for storing the dataframes of motif frequencies for each web
...
```
**Figure S1: Robustness of quasi sign-stability to different distributions used to fill the random Jacobian matrix. Facet labels represent the maximum impact of the prey on the predator population (positive number) and maximum impact of the predator on the prey population (negative number)**
...
## Figure 2
```{r echo = F}
z.both <- read.csv("C:/Users/jjborrelli/Desktop/GitHub/Subgraph-Stability/zboth.csv", row.names = 1)
```
